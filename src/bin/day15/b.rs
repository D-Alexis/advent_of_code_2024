...
fn find_boxes_to_move(pos: &mut (usize,usize), moves: &(i32,i32), map: &mut Vec<Vec<char>>) ->  Option<HashSet<(i32,i32)>> {
...
fn check_a(map: &mut Vec<Vec<char>>, next :(i32,i32), boxes_to_check: &mut VecDeque<(i32,i32)>) {
    if map[next.0 as usize][next.1 as usize] != '.' { // pas un . ni un # donc [ ou ]
        if map[next.0 as usize][next.1 as usize] == '[' {
            boxes_to_check.push_back(next);
        } else { // ']'
